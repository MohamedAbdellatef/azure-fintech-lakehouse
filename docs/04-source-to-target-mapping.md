...
*(Note: `phone_number`, `date_of_birth`, `gender`, `created_at`, `updated_at` are dropped).*
...
| `user_sk` | `TXN.sender_account_id` | Derive `user_id` via `silver_accounts` (`sender_account_id -> user_id`), then as-of join to `dim_user` where `TXN.transaction_timestamp_utc` is between `valid_from` and `COALESCE(valid_to,'9999-12-31')` |
